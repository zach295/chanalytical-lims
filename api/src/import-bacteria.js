...
app.http('import-bacteria', {
  ...
  handler:   async (request, context) => {
    try {
      ...
      // Filter to non-rejected items with Lab IDs
      const cacheItems = rcItems.filter(i => i.fields?.LabID && !/\bREJ\b/i.test(i.fields.LabID));
      if (!cacheItems.length) {
        return { status: 200, jsonBody: { success: true, updated: 0, log: ['No Results Cache items to process'] } };
      }

      // Group by MMDDYY date prefix
      const byPrefix = {};
      for (const item of cacheItems) {
        const labId = String(item.fields.LabID || '').trim();
        const prefix = labId.slice(0, 6);
        if (/^\d{6}$/.test(prefix)) {
